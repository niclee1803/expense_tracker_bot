...
async def begin_amount_input(
    ...
    if mode == "expense":
        ...
        placeholder = "25.60 mcdonalds lunch"
    else:
        prompt = (
...
async def start(
    ...
    budget = get_budget_cents(
        telegram_user.id,
        current_month,
    )

    if not snapshot.exists or budget is None:
        await update.message.reply_text(
            f"Hi {escape(telegram_user.first_name or 'there')}! 👋\n\n"
            "Let's set up your expense tracker. "
            "First, what is your monthly budget?",
            parse_mode=ParseMode.HTML,
        )

        await begin_amount_input(
            update,
            context,
            "setup_budget",
        )
    ...
    await update.message.reply_text(
        f"Welcome back, "
        f"{escape(telegram_user.first_name or 'there')}! "
...
async def handle_amount(
    ...
    try:
        if mode == "expense":
            cents, comment = parse_expense_input(
                update.message.text
            )
        else:
            cents = parse_money_to_cents(
                update.message.text
            )
            comment = ""
        ...
        if mode == "expense":
            example = (
                "<code>25.50</code> or "
                "<code>25.50 mcdonalds lunch</code>"
            )
            placeholder = "25.50 mcdonalds lunch"
        else:
            example = "<code>1000.00</code>"
            placeholder = "Enter budget, e.g. 1000.00"

        await update.message.reply_text(
            f"Please enter a valid amount greater than zero, "
            f"such as {example}. "
            "Comments must be 200 characters or fewer. "
            "Use /cancel to stop.",
            parse_mode=ParseMode.HTML,
            reply_markup=ForceReply(
                selective=True,
    ...
    if mode in {"setup_budget", "adjust_budget"}:
        ...
        user_ref(user_id).collection(
            "budget_changes"
        ).document(current_month).set(
            {
        ...
        verb = (
            "set"
            if mode == "setup_budget"
            else "updated"
        )

        if remaining >= 0:
            remaining_line = (
                f"Remaining for {month_title(current_month)}: "
                f"<b>{money(remaining)}</b>"
            )
        else:
            remaining_line = (
                f"Over budget for {month_title(current_month)} by: "
                f"<b>{money(abs(remaining))}</b>"
            )

        await update.message.reply_text(
            f"✅ Your monthly budget is {verb} to "
            f"<b>{money(cents)}</b>.\n"
            f"{remaining_line}",
            parse_mode=ParseMode.HTML,
            reply_markup=MAIN_MENU,
    ...
    if mode == "expense":
        ...
        context.user_data["pending_expense_comment"] = comment
    ...
    await update.message.reply_text(
        "This entry expired. Please start again.",
        reply_markup=MAIN_MENU,
    )
...
async def category_selected(
    ...
    cents = context.user_data.pop(
        "pending_expense_cents",
        None,
    )
    comment = context.user_data.pop(
        "pending_expense_comment",
        "",
    )

    if cents is None or category not in CATEGORIES:
        await query.edit_message_text(
            "This expense entry expired. "
            "Please add it again."
        )
        return
    ...
    # Do not create a comment field when none was entered.
    if comment:
    ...
    if remaining < 0:
        status = "⚠️ You are over budget by"
        remaining_text = money(abs(remaining))
    else:
...
def expense_report(
    ...
    lines = [
        f"📊 <b>{month_title(target_month)}</b>",
        "",
    ]
...
async def show_past_month_menu(
    ...
    rows = [
        [
            ...
            InlineKeyboardButton(
                "Past 6 months",
                callback_data="period:6",
            ),
        ],
        [
            InlineKeyboardButton(
                "Past year",
                callback_data="period:12",
            )
        ],
    ]
...
async def past_period_selected(
    ...
    try:
        month_count = int(
            query.data.removeprefix("period:")
        )
    except ValueError:
        await query.edit_message_text(
            "That period is invalid."
        )
        return
    ...
    await query.message.reply_photo(
        photo=chart,
        caption=(
            f"Category spending for the past "
            f"{month_count} months, including this month."
        ),
    )

    await query.edit_message_text(
        f"Showing your past {month_count} months."
    )
...
async def past_month_selected(
    ...
    target_month = query.data.removeprefix(
        "month:"
    )

    if target_month not in previous_months(12):
        await query.edit_message_text(
            "That month is no longer available "
            "from this menu."
        )
        return
...
async def show_delete_expense_menu(
    ...
    if not expenses:
        await update.message.reply_text(
            "You have no expenses to delete "
            "this month.",
            reply_markup=MAIN_MENU,
        )
        return
...
async def delete_expense_selected(
    ...
    expense_id = query.data.removeprefix(
        "delete_pick:"
    )
    expense_ref = (
        user_ref(query.from_user.id)
        .collection("expenses")
        .document(expense_id)
    )
    ...
    await query.edit_message_text(
        f"Delete "
        f"<b>{money(int(expense['amount_cents']))}</b> "
        f"from {escape(category_label)}?"
        f"{comment_line}",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(
            [
...
async def delete_expense_confirmed(
    ...
    if not snapshot.exists:
        await query.edit_message_text(
            "That expense was already deleted."
        )
        return
...
async def generate_xlsx(
    ...
    await update.message.reply_document(
        ...
        caption=(
            "Your expense data, with one worksheet "
            "for each month containing expenses."
        ),
        reply_markup=MAIN_MENU,
    )
...
async def menu_message(
    ...
    if text == ADD_EXPENSE:
        ...
        await show_delete_expense_menu(
            update,
            context,
        )

    elif text == VIEW_REMAINING:
        await show_remaining(
            update,
            context,
        )

    elif text == VIEW_CURRENT:
        await show_current_expenses(
            update,
            context,
        )

    elif text == VIEW_PAST:
        await show_past_month_menu(
            update,
            context,
        )
        ...
        await update.message.reply_text(
            ...
            f"<b>{money(current_budget)}</b>.\n"
            "Enter a new monthly budget. "
            "It will apply from this month onward.",
            parse_mode=ParseMode.HTML,
        )
...
def main():
    if not BOT_TOKEN:
        raise RuntimeError(
            "Set TELEGRAM_BOT_TOKEN in your "
    ...
    application.add_handler(
        CallbackQueryHandler(
            ...
            pattern=r"^delete_pick:",
        )
    )
    application.add_handler(
        CallbackQueryHandler(
            delete_expense_confirmed,
            pattern=r"^delete_confirm:",
        )
    )
    application.add_handler(
        CallbackQueryHandler(
            delete_expense_cancelled,
